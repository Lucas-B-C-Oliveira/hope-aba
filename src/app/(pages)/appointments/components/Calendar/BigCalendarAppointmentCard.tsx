...
export const BigCalendarAppointmentCard = memo(function BigCalendarAppointmentCard({
  dataAppointment,
}: Props) {
  console.log(
  ...
  const { data: appointmentData, refetch } = useQuery({
    queryKey: ['getAppointmentData'],
    queryFn: async () => {
      const { data } = await doFetch(`appointments/${dataAppointment?.id}`)
      const newData = {
        ...data,
        status: getStatusName(data?.status)
      }
      return newData
    }
  })
  ...
  async function handleChangeStatus(newStatus: 'cancel' | 'confirm' | 'done') {
    setLoading(true)
    const result = await doFetch(`appointments/${appointmentData?.id}/${newStatus}`, {
      method: 'PATCH'
    })
    setLoading(false)
    refetch()
  ...
  return (
    ...
    >
      <div
        className="flex flex-col gap-4 h-full w-full justify-between items-start"
      >
        ...
        <div className="flex flex-row gap-4">
          ...
          <div className="flex flex-col gap-2 rounded-md border-0   items-center py-2 pt-3 px-2 box-content  shadow-sm ring-1 ring-inset ring-gray-300">
            ...
            <div className="flex flex-col gap-2">
              <ActionButton onClick={() => handleChangeStatus("cancel")} classNameToMerge="bg-red-600 hover:bg-red-500 w-24 items-center justify-center px-0">
                {!loading && ("Cancelar")}
                {loading && ("Loading")}
              </ActionButton>

              <ActionButton onClick={() => handleChangeStatus("confirm")} classNameToMerge="bg-sky-600 hover:bg-sky-500 w-24 items-center justify-center px-0">
                {!loading && ("Confirmar")}
                {loading && ("Loading")}
              </ActionButton>

              <ActionButton onClick={() => handleChangeStatus("done")} classNameToMerge="bg-emerald-600 hover:bg-emerald-500 w-24 items-center justify-center px-0">
                {!loading && ("Realizado")}
                {loading && ("Loading")}
              </ActionButton>
            </div>
