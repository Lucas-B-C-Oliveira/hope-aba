...
export function AutocompleteInput({ loading, onSearchChange, currentOptions }: Props) {

  const [isFocused, setFocus] = useState(false);

  return (
    <div tabIndex={0} className={twMerge(MAGIC_INPUT_CLASSNAME, 'rounded-[0.290rem]  w-56 cursor-default pb-2 items-center flex flex-row gap-1 pr-2 pl-3 justify-between',
      `${isFocused ? 'ring-inset ring-2 ring-indigo-600' : 'hover:ring-2  hover:ring-inset hover:ring-indigo-600'}`)}>
      <Combobox.Input
        className={twMerge(
