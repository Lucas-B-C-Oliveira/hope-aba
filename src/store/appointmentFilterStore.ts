...
export const useAppointmentFilterStore = create<AppointmentFilterStore>()((set, get) => ({
  filters: {
  ...
  addFilter: (filterKey, newFilterValue) => {
    ...
    if (filterKey !== 'rooms' && filterKey !== 'therapies') {
      ...
      if (typeof currentFilters !== 'undefined' && Array.isArray(currentFilters)) {
        const newFilterAlreadyIsInState = currentFilters?.some((filter: Filter | undefined) => filter?.id === newFilterValue?.id)
  ...
  removeFilter: (filterKey, filterToRemoveId) => {
    ...
    if (filterKey !== 'rooms' && filterKey !== 'therapies') {
      ...
      console.log('_____________ aaaaaa ___________ currentFilterState', currentFilterState)
    }
    else {
      if (typeof filterToRemoveId !== 'undefined' && Array.isArray(currentFilters)) {
        const newArray = currentFilters.filter((filterValue) => filterValue?.id !== filterToRemoveId)
        const newFilters = {
          ...currentFilterState,
          [filterKey]: newArray
        }
